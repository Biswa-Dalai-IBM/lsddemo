...
/**
 * Resolves variable paths using scope information and creates NSField/NSRecord structures.
 * This class bridges the gap between parsed variable declarations and webMethods IData structures.
 */
public class VariableResolver {
	...
	/**
	 * Create an IData structure for a variable path based on its type
	 * This can be used to create NSField/NSRecord structures
	 * @param path Variable path
		...
		if (param.hasChildren()) {
			...
			for (ParameterDeclaration child : param.getChildren()) {
				info.append("    - ").append(child.getName())
					.append(" (").append(child.getDataType()).append(")\n");
			}
		}
